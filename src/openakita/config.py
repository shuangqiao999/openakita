...
class Settings(BaseSettings):
    ...
    # === 异步与并行优化 ===
    async_task_timeout: int = Field(default=30, description="异步任务超时时间（秒）")

    parallel_tool_workers: int = Field(default=4, description="并行工具执行的工作线程数")

    parallel_search_engines: int = Field(default=3, description="并行搜索的搜索引擎数量")

    websocket_reconnect_attempts: int = Field(default=5, description="WebSocket 最大重连次数")

    websocket_reconnect_delay: int = Field(default=1, description="WebSocket 重连初始延迟（秒）")

    llm_retry_count: int = Field(default=3, description="LLM 调用失败重试次数")

    llm_retry_delay: float = Field(default=1.0, description="LLM 重试间隔（秒）")

    structured_output_schema: str = Field(
        default="intent", description="结构化输出 Schema 类型: intent / memory / selfcheck"
    )

...
FILESYSTEM_TOOLS = [
    {
        ...
        "description": (
            ...
            "Working directory and env persist across calls (use working_directory param, not cd).\n\n"
            "Prefer specialized tools over shell equivalents: read_file>cat, write_file>sed, grep>find, "
            "web_fetch>curl, move_file>mv. On Windows, prefer run_powershell; use run_shell only for bash/Git Bash.\n\n"
            "Long commands: set block_timeout_ms=0 for servers/watchers. Output >200 lines auto-truncated; "
            "full output readable with read_file. Chain dependent commands with &&; run independent commands in parallel.\n\n"
            "Kill hung background processes: run_shell(command='kill {pid}'). "
            "Monitor background output: read terminal file at data/terminals/{session_id}.txt."
        ),

...
- **每轮结束前必须检查并保存重要事实**：若本轮用户透露了可用于未来回忆的信息（个人事实、偏好、重要决策、任务背景），在返回最终响应前调用 `add_memory` 保存 ≥1 条相关记忆，type='fact'，scope='user'。这确保了跨天上下文连贯性。不得仅凭短期对话上下文假设信息已经记住。
...
def _build_pinned_rules_section(
    ...
    try:
        ...
        if query_visible:
            ...
            rules = store.query_semantic(memory_type="rule", scope=None, limit=20)
        if not rules:
            return ""
    ...
    for t in exp_types:
        try:
            ...
            if query_visible:
                ...
                results = store.query_semantic(memory_type=t, scope=None, limit=10)
            else:
                results = []

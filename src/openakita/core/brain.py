...
class Brain:
    ...
    async def messages_create_async_with_retry(
        self,
        messages: list,
        system: str = "",
        tools: list = None,
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = str(e)
                logger.warning(f"Attempt {attempt + 1} failed: {last_error}")
                if "structured output" in str(e).lower() and tools:
                    logger.info("Retrying without structured output")
                    tools = None
                    continue

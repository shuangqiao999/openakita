    ...
    def _effective_tools(self) -> list[dict]:
        ...
        if intent_hints and hasattr(self, "tool_catalog"):
            ...
            if deferred_cats:
                ...
                if deferred_names:
                    before = len(tools)
                    tools = [t for t in tools if t.get("name") not in deferred_names]
                    if len(tools) < before:
                        logger.info(
...
- 对话历史中的 [子Agent工作总结] 和 [执行摘要] 是已完成的事实，不要重复执行"""
        ...
        if session and len(session_messages) >= 4:
            ...
            if topic_changed:
                _boundary_msg = {
                    "role": "user",
                    "content": (
                        "[上下文边界] 检测到话题切换，以下是新话题。请优先关注边界之后的内容。"
                    ),
    ...
    async def chat_with_session(
        ...
        *,
        ...
        # === 轻量级预检查 (0 Token, <10ms) ===
        # 在 IntentAnalyzer 之前快速处理简单对话和工具调用
        if hasattr(self, "lightweight_agent") and self.lightweight_agent:
            try:
                _lw_result = self.lightweight_agent.process(message, session_id)
                if _lw_result["type"] == "reply":
                    logger.info(
                        f"[LightweightAgent] Fast reply (no LLM): {_lw_result['content'][:50]}..."
                    )
                    return _lw_result["content"]
                elif _lw_result["type"] == "tool_call":
                    logger.info(f"[LightweightAgent] Fast tool routing: {_lw_result['tool_name']}")
                    # TODO: 快速工具调用路径（后续集成 ToolExecutor）
                    # 目前记录日志，交给完整流程处理
            except Exception as e:
                logger.debug(f"[LightweightAgent] Pre-check failed: {e}")
